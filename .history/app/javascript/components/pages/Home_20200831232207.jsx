...
class Home extends React.Component {
  ...
  handleSubmit = async (event) => {
    ...
    try {
      ...
      const errorMessage = error.response.request.responseText
        .split(":")[1]
        .replace(/["}]/g, "");
  ...
  fetchListOfUrls = async () => {
    try {
      ...
      this.setState({
        ...this.state,
        arrayOfUrls: response.data.urls,
      });
    } catch (error) {
      this.setState({
        ...this.state,
        errors: response.data.urls,
      });
      console.error(error);
    }
  };
      ...
      <React.Fragment>
        <div className="pb-5">
          ...
          <section>
            <div className="wrapper py-3">
              <form id="createUrlForm" onSubmit={this.handleSubmit}>
                ...
                />
                <button className="w-25 bg-navy-blue text-pink">Submit</button>
              </form>
            </div>
          </section>
          ...
          <div className="wrapper ">
            <div className="bg-navy-blue d-flex justify-content-around align-items-center  pl-3 text-white py-3 mt-3 border-bottom border-dark">
              <p className="my-0 pl-5 text-pink">Original</p>
              <p className="my-0 text-purple">Shortened</p>
            </div>
            <DisplayUrl
              urls={this.state.arrayOfUrls}
